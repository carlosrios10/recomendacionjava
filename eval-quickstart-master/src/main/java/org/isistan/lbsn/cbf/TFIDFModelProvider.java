    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                                            ...
                                            .get()) {
                String tagLine = tagApplication.get(TagData.TAG);
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            double sumSq = 0.0;
            double totalTerm = tv.size();
            // TODO Convert this vector to a TF-IDF vector
            for (Map.Entry<String, Double> e:tv.entrySet()){
                double tf_idf = (e.getValue()/totalTerm)*docFreq.get(e.getKey());
                sumSq = sumSq + (tf_idf*tf_idf);
                e.setValue(tf_idf);
            }
            ...
            double euclideaNorm = Math.sqrt(sumSq);
            for (Map.Entry<String, Double> e:tv.entrySet()){
                double tf_idf_norm = e.getValue()/euclideaNorm;
                e.setValue(tf_idf_norm);
            }
