...
public class NeighborhoodFinderZonaDBscan implements NeighborFinder {
	private static final Logger logger = LoggerFactory.getLogger(NeighborhoodFinderZonaDBscan.class);
    private final DataAccessObject dao;
    private  double neighborhoodRadio;
     ...
     */
    @Inject
    public NeighborhoodFinderZonaDBscan(RatingVectorPDAO rvd, DataAccessObject dao,UserSimilarity sim,UserVectorNormalizer norm,@UserSimilarityThreshold Threshold thresh,@DistBetweenZonas double radioNeig) {
        similarity = sim;
        normalizer = norm;
        rvDAO = rvd;
        this.dao = dao;
        threshold = thresh;
        neighborhoodRadio = radioNeig;
        calcularZonas();
    ...
    private void calcularNeigborCercanos() {
    	logger.info("Calcular Neigbor Cercanos DBScan");
    	...
    	logger.info("Fin Calcular Neigbor Cercanos DBScan");
    	zonas = null;
    }
    
    
    private void calcularZonas(){
    	logger.info("Calculando Zonas DBScan");
    	LongSet usersIds = dao.getEntityIds(CommonTypes.USER);
    	for (Long user : usersIds) {
			...
			List<CentroidCluster<EntityItemWrapper>> centroids = new ArrayList<CentroidCluster<EntityItemWrapper>>();
			
			for (Iterator<Cluster<EntityItemWrapper>> iterator = clusterResults.iterator(); iterator.hasNext();) {
				Cluster<EntityItemWrapper> cluster = iterator.next();
				CentroidCluster<EntityItemWrapper> cent = new CentroidCluster<EntityItemWrapper>(new EntityItemWrapper(calcularCentroid(cluster)));
				centroids.add(cent);
			}
			
			zonas.put(user, centroids);
		}
    	logger.info("Se calcularon DBScan zonas para {} usuarios ",zonas.size());
    }
    ...
    private double []  calcularCentroid(Cluster<EntityItemWrapper> cluster) {
    	List<EntityItemWrapper> points  = cluster.getPoints();
	    double centroidLatitude = 0;
	    double centroidLongitude = 0;
	    for (Iterator<EntityItemWrapper> iterator = points.iterator(); iterator.hasNext();) {
			EntityItemWrapper entityItemWrapper = iterator.next();
