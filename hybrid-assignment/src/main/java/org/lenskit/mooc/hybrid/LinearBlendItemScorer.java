...
public class LinearBlendItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        for (Long item:items){
            double bias = this.biasModel.getIntercept() + this.biasModel.getUserBias(user) + this.biasModel.getItemBias(item);
            Result leftScore = this.leftScorer.score(user,item);
            double leftComponetScore = (leftScore == null)?0:((1-this.blendWeight)*(leftScore.getScore() - bias));
            Result rigthScore = this.rightScorer.score(user,item);
            double rigthComponentScore = (rigthScore == null)?0:(this.blendWeight*(rigthScore.getScore() - bias));

            double finalScore = bias + leftComponetScore + rigthComponentScore;

            results.add(new BasicResult(item,finalScore));
        }
