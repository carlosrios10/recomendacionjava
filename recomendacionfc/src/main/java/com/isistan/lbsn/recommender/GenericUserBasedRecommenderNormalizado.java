...
public class GenericUserBasedRecommenderNormalizado extends AbstractRecommender implements UserBasedRecommender{
	private static final Logger log = LoggerFactory.getLogger(GenericUserBasedRecommenderNormalizado.class);
	  ...
	  protected float doEstimatePreference(long theUserID, long[] theNeighborhood, long itemID) throws TasteException {
	    ...
	    int count = 0;
	    String userItemVecinos = theUserID+":"+itemID+":vecinos:";
	      ...
	      if (userID != theUserID) {
	        ...
	        if (pref != null) {
	          ...
	          if (!Double.isNaN(theSimilarity)) {
	        	userItemVecinos = userItemVecinos+","+userID+","+pref+","+theSimilarity;
...
//	    String userItemVecinos = theUserID+":"+itemID+":"+estimate+":";
//	    for (int i = 0; i < theNeighborhood.length; i++) {
//	    	userItemVecinos =  userItemVecinos + ","+theNeighborhood[i];
//		}
	    userItemVecinos = userItemVecinos+":"+estimate;
	    log.debug(userItemVecinos);
	    return estimate;
	  }
